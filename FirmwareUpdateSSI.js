...
module.exports = {
    main: async function (data, userID,version,FirmwareUpdatePort,ForceDownloading) {
        try {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                Unit.EventCode =  4141,
                Unit.EventData = getBytes(settings,version,FirmwareUpdatePort,ForceDownloading);
...
function getBytes(data,version,FirmwareUpdatePort,ForceDownloading) {
    ...
    let DeviceSelectionByte = Converter.byte(04);
    bytearray = Buffer.concat([bytearray, DeviceSelectionByte]);

    let ForceDownloadingByte = Converter.byte(ForceDownloading);
    bytearray = Buffer.concat([bytearray, ForceDownloadingByte]);
