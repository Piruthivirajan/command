	...
	main: async function (data, userID,PrevisousData) {
		try {
			...
			for (let i = 0; i < data.length; i++) {
					...
					settings.code = data[i].MISCSettingsJson["cmd_10B3"]["Code"]
					if(PrevisousData!=null && PrevisousData.MISCSettingsJson["cmd_10B3"]["Code"]==settings.code){
...
function getBytes(data) {
	...
	//for(let i=0;i<data.code.length;i++){
		let code =  Converter.Integer(data.code)
		byte_array = Buffer.concat([byte_array,code]);
	//}
