		...
		try {
			...
			for (let i = 0; i < data.length; i++) {
				...
				if(PrevisousData!=null){
					//PrevisousData.IdlingOptionEnabled=PrevisousData.IdlingOptionEnabled?"Y":"N"
				}
				if(PrevisousData!=null && PrevisousData.IdlingOptionEnabled==data[i].IdlingOptionEnabled
					&& PrevisousData.IdlingAllowedTimeToChoose==data[i].IdlingAllowedTimeToChoose
					&& PrevisousData.IdlingOption1==data[i].IdlingOption1
					&& PrevisousData.IdlingOption2==data[i].IdlingOption2
					&& PrevisousData.IdlingOption3==data[i].IdlingOption3
					&& PrevisousData.IdlingOption4==data[i].IdlingOption4
					){
					continue
				}
				
				var settings = {};
				settings.Enabled = data[i].IdlingOptionEnabled == "Y"
				settings.ChoosingTime = (data[i].IdlingAllowedTimeToChoose == null ?0 : (data[i].IdlingAllowedTimeToChoose));
				settings.Option1 = data[i].IdlingOption1
				settings.Option2 = data[i].IdlingOption2
				settings.Option3 = data[i].IdlingOption3
				settings.Option4 = data[i].IdlingOption4
