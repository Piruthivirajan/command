	...
	main: async function (data, userID,PrevisousData) {
		try {
			...
			for (let i = 0; i < data.length; i++) {
					...
					settings.Seconds = data[i].MISCSettingsJson["cmd_110D"]["Seconds"]				
					if (PrevisousData != null && PrevisousData.MISCSettingsJson["cmd_110D"]["Seconds"] == data[i].MISCSettingsJson["cmd_110D"]["Seconds"]) {
						continue;
					}
...
function getBytes(data) {
	...
	let Seconds =  Converter.byte(data.Seconds)
	byte_array = Buffer.concat([byte_array,Seconds]);
