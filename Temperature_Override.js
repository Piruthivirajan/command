	...
	main: async function (data, userID, PrevisousData) {
		try {
			...
			for (let i = 0; i < data.length; i++) {
				...
				if (PrevisousData != null && PrevisousData.MISCSettingsJson["cmd_110F"]["Enabled"] == data[i].MISCSettingsJson["cmd_110F"]["Enabled"]) {

					continue;

				}
				settings.Enabled = data[i].MISCSettingsJson["cmd_110F"]["Enabled"]
