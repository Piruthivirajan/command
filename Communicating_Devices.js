	...
	main: async function (data, userID,PrevisousData) {
		try {
			...
			for (let i = 0; i < data.length; i++) {
					...
					if (PrevisousData != null && PrevisousData.MISCSettingsJson["cmd_1112"]["Devices"] == data[i].MISCSettingsJson["cmd_1112"]["Devices"]) {

						continue;
	
					}

					settings.Devices = data[i].MISCSettingsJson["cmd_1112"]["Devices"]
					if(settings.Devices!=''){
					settings.Devices=settings.Devices.split(",").reduce(function(a, b){
...
function getBytes(data) {
	...
	let Devices =  Converter.byte(data.Devices)
	byte_array = Buffer.concat([byte_array,Devices]);
